...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      const redirect = router.query.redirect ? decodeURIComponent(router.query.redirect) : '/dashboard';
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="mt-4 text-xl font-bold text-white">Operator Console Sign In</h2>
          <p className="mt-1 text-xs text-slate-400">Authenticate to manage automations and live agent queues</p>
        </div>
        ...
        <div className="glass-panel p-8 rounded-3xl border border-slate-800 shadow-2xl">
          ...
          <div className="mt-6 pt-5 border-t border-slate-800 text-center">
            <p className="text-xs text-slate-400">
              ...
              <Link href="/register" className="text-brand-400 hover:text-brand-300 font-semibold transition">
                Create Account
              </Link>
            </p>
          </div>
